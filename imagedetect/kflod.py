...
def kfold(src_path,
    ...
    def get_confusion_matrix_intervals(pred, target,level=0.8):
        ...
        unique_labels = sorted(set.union(*[set(tuple(single) for single in interval) for interval in pred_intervals] + [set(target.astype('int').tolist())]))
        ...
        for i in range(len(target)):
            for pred_label in pred_intervals[i]:
                matrix[target[i]][pred_label] += 1
